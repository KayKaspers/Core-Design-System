...
harness matches, 25/25 DE/EN terminology, Candidate Dossier) — **Experimental at
implementation, committed, executor-produced, no Candidate status then, and still
no visual value**. That WP-013 and WP-015 evidence was then **independently reviewed
by CDS-WP-016**: **Independent Review PASS**, **Candidate Recommendation GO** — and
**GO is not a Candidate award**.

**The Candidate authority gates have since closed.** Fresh revision-bound evidence
**`AE1-CDS-WP016-SEMSTATUS-004`** was produced, independently reviewed (**PASS WITH
NOTES**), and **admitted at AE-1** by the Human Maintainer; the **Nova Candidate
Finalization Review returned GO WITH NOTES**; the Human Maintainer **approved
Candidate**; and the exact-byte **Promotion Commit
`22fa0710e2b75df22e7b420c2f9d86bbe67b2777`** made it effective on 2026-08-19 —
gate **PASS**, committed blob identity **15/15 exact**, regression
**47/47 · 64/64 · 184/184 · 24/24/0/0**.

**Current Semantic Status state:** source revision
**`semantic-status-rev-0002-candidate`** · maturity **`Candidate`** · approval
**`Approved`** · **Candidate YES** · admitted evidence in force
**`AE1-CDS-WP016-SEMSTATUS-004`** at **AE-1**, channel-independent source/contract
...
admitted**. **Stable No** · claims **none** · conformance **none** · every other
...
`1fc53ae5afa40807e1950171ab700b0860ee581e`; **CDS-WP-017 is `Completed`**, closed
...
`538fbccbf6f554de3b872e9fb75a70d13318feb6`. **CDS-WP-020 — Reference and Semantic
Token Foundation is `Completed`**: authorized separately on 2026-08-26, **executed
with result `DECISION_REQUIRED`**, and **integrated** by the Human-Maintainer commit
...
NOTES`**, **integrated** by the Human-Maintainer exact-object commit
...
**`INTEGRATED ≠ CLOSED`** and **`CLOSURE OBJECT PREPARED ≠
CLOSURE EFFECTIVE`** each held until that commit. The execution result is unchanged by closure and is not
...
and governance boundaries. It selects **no colour, typeface, size, spacing, radius,
stroke, shadow, opacity, icon, illustration, motion value, breakpoint, or theme**;
creates **no** token source file, schema, validator rule, component, brand, or
Product Profile; produces and admits **no** evidence; changes **no** maturity;
registers **no new Decision, ADR, or risk**; **renames no phase**; registers **no
capability**; and activates **no** later work package. **No visual value exists in
CDS**, and all nine visual foundation families are **`Proposed`**.

CDS-WP-020 is **contract only.** It defines what a visual **reference primitive**
and a visual **semantic role** are, what each must declare, how a role aliases a
primitive, and the discipline any future value must satisfy — the
...
It selects **no value** and creates **no identifier**, **no** token source file,
schema, validator rule, or test; produces and admits **no** evidence; changes **no**
maturity; and registers **no new risk**. Its **value and
machine-readable** half is gated on seven normative choices no committed CDS source
had made — **OD-1 … OD-7**, held in the **non-normative**
[Visual Token Foundation Open Decisions](../docs/roadmap/VISUAL_TOKEN_FOUNDATION_OPEN_DECISIONS.md)
register.

A separately authorized **Decision Integration Pass** (2026-08-27) **prepared**
**DEC-S-128** (one canonical `srgb` colour representation; OKLCH derivational only),
**DEC-S-129** (**WCAG 2.2** contrast evaluation authority; full-precision
comparison; APCA informational only), **DEC-S-130** (an explicit, minimal, closed
`$type` admission profile — `color`, `dimension`, `number`; **`profileVersion` stays
`1`**), **DEC-S-131** (the **Source Set** as the independently evaluable unit;
**AGGREGATED is not MATURE**) and **ADR-0004**. **All five are effective** at the
Human-Maintainer integration commit `42a568d8…`, they select **no** value and
...
concrete machine-readable Visual Token Source and Value Authoring work is routed to
...
the currently authorized work package**. **`CLOSED ≠ SUCCESSOR AUTHORIZED`** and
**`DEPENDENCY SATISFIED ≠ AUTHORITY GRANTED`**: the CDS-WP-022 authorization was a
separate, explicit Human-Maintainer act, and **it authorizes nothing further** —
**`CDS-WP-020A` gains no prerequisite from it.** The single active forward roadmap is the
[Post-Candidate Development Roadmap](../docs/roadmap/POST_CANDIDATE_DEVELOPMENT_ROADMAP.md).

## Theme and environmental presentation model (CDS-WP-022)

*(Compact orientation. The normative source is the
[Visual Foundation Theme Architecture](../docs/architecture/VISUAL_FOUNDATION_THEME_ARCHITECTURE.md);
this is a **summary, never a normative source**.)*

**Contract only. Result: `COMPLETE WITH NOTES`, after first returning
`DECISION_REQUIRED` — which stands as execution history.**

- **A Theme Resolution Context is a named presentation condition** under which
  approved semantic roles resolve to approved primitives, with **no** change to what a
  role means or whether it exists. **"Theme" and "Theme Resolution Context" are the
  same construct.**
- **`CANDIDATE ≠ SUPPORTED CONTEXT`.** **CA-1 … CA-13** state what a decision to
  support a context must affirm; a context is admitted **only** by an explicit
  Human-Maintainer decision, as an **Elevated** change.
- **A context is never a path segment.** **CI-1 … CI-6** keep it out of token paths,
  Source Set identifiers and the layer field — **no `context` or `theme` namespace
  segment exists at any position** (DEC-S-132 clause 12).
- **The mechanism is decided: a Resolver-Modifier Context** over the existing
  Source-Set graph (**`DEC-S-137`**, **TM-1 … TM-12**). Product Profile and
  per-context token path **excluded**; per-context Source Set **unavailable** under
  DEC-S-131 clause 5 and DEC-S-132 clauses 10 and 12; **the Resolver never becomes a
  second maturity unit**. **The chosen architecture amends nothing.**
- **Context evidence is an input binding.** Evidence stays bound to (`sourceSetId`,
  `sourceRevision`) and records the **Resolver / Composition revision** and the
  **Theme Resolution Context** as **exact inputs** (**TM-7**): **`EVIDENCE INPUT ≠
  MATURITY CARRIER`**, and **any change to either invalidates or supersedes the
  affected evidence** (**TM-9**).
- **Supported Core contexts: `Light` and `Dark`, equal peers, no default** — **from
  the effectivity of `DEC-S-138` and not before; the set is empty until then**. They
  are **human-readable architectural names, not machine-readable identifiers**.
- **Forced colours and platform high contrast: an environmental accessibility
  condition, not a Core context** — binding **independently of the selected Theme**,
  and **an architecture obligation, never evidence or conformance**.
  **`ACCESSIBILITY OBLIGATION ⇏ CDS SHIPS A HIGH-CONTRAST THEME`.**
- **Selection is the consumer's; resolution is CDS's** (**CS-1 … CS-11**).
  **Explicit viewer choice > inferred environment preference**; mandatory platform
  accessibility conditions **outside** Theme precedence and always binding; **no
  technology named**; **no CDS artifact stores viewer or environment state**.
  **`DETERMINISTIC RESOLUTION ≠ DETERMINISTIC SELECTION`.**
- **No default, no fallback, fail closed** (**CF-1 … CF-11**), **no silent
  substitution**, **no automatic repair**, and **`Not Applicable`** where Theme
  resolution genuinely does not apply — never as a hidden default.
...
- **Registers: 136 decisions · 6 ADRs until that commit; 138 · 7 from it; 98 risks
  ...
  **AE-0**; **claims None · conformance None · Stable No**; publication **`Private
  ...
  and **`THEME GATE SATISFIED ≠ VALUE SELECTION AUTHORIZED`**. **CDS-WP-022 is not
  closed.**
...
| **Visual foundation architecture** (CDS-WP-019 — structure only, **no visual value**) | [docs/architecture/VISUAL_FOUNDATION_ARCHITECTURE.md](../docs/architecture/VISUAL_FOUNDATION_ARCHITECTURE.md) — entry point to the colour, typography, spatial, shape and surface, iconography and imagery, and theme architectures, and to the accessibility, channel, brand and profile, and governance mappings |
| **Visual reference and semantic token contract** (CDS-WP-020 — contract only, **no value, no identifier**) | [docs/architecture/VISUAL_REFERENCE_TOKEN_FOUNDATION.md](../docs/architecture/VISUAL_REFERENCE_TOKEN_FOUNDATION.md) · [docs/architecture/VISUAL_SEMANTIC_TOKEN_FOUNDATION.md](../docs/architecture/VISUAL_SEMANTIC_TOKEN_FOUNDATION.md) · [docs/governance/VISUAL_TOKEN_VALUE_SELECTION_RULES.md](../docs/governance/VISUAL_TOKEN_VALUE_SELECTION_RULES.md) — the open decisions that gate every value are held **non-normatively** in [docs/roadmap/VISUAL_TOKEN_FOUNDATION_OPEN_DECISIONS.md](../docs/roadmap/VISUAL_TOKEN_FOUNDATION_OPEN_DECISIONS.md) |
...
- DEC-S-128…131: visual token representation, evaluation authority and source
  identity decisions (CDS-WP-020 Decision Integration Pass) — DEC-S-001…127
  unchanged. **Effective at commit `42a568d8…`.**
- ADR-0001 (Machine-Readable Token Source Format), ADR-0002 (Deterministic JSON
  ...
  exist and are effective. **ADR-0004 (Visual Token Representation and Source
  Identity Architecture) is `Accepted` and effective** at commit `42a568d8…`,
  bringing the effective range to 4.
...
- Range: RISK-001 … RISK-098 · Count: 98 · **89 Monitored; RISK-031, RISK-040,
  RISK-044, RISK-066, RISK-067, RISK-068, RISK-069, RISK-071, RISK-098 Mitigating**
...
| RISK-097 | Semantic Status source consumed prematurely — still live after the Candidate promotion: Candidate is not Stable, release, publication, adoption, consumer authority, or conformance. |
...
executor-produced 24/24 evidence, not Candidate at that milestone). That WP-013 and
WP-015 evidence was then **independently reviewed by CDS-WP-016** — **Independent
Review PASS**, **Candidate Recommendation GO** — and **GO is not a Candidate
award**: at that point Candidate was still **No**, maturity **Experimental**,
approval **Unapproved**. The authorized work package is **CDS-WP-016 — Semantic
...
limitations). After a **fresh independent review of the remediation and of its
clean-HEAD evidence package** (**PASS WITH NOTES** and **PASS**), the Human
Maintainer **admitted `AE1-CDS-WP016-SEMSTATUS-002` at AE-1** on 2026-08-17 — for
the channel-independent Semantic Status source/contract family **only**, bound to
`semantic-status-rev-0001`; every other artifact remains **AE-0**. A further
...
authoritative source revision was still `semantic-status-rev-0001`, and **that
rework produced no Candidate evidence, admitted none, and granted no Candidate
approval**.

**The remaining authority steps have since completed.** Fresh revision-bound
...
`538fbccbf6f554de3b872e9fb75a70d13318feb6`. **CDS-WP-020 — Reference and Semantic
Token Foundation** (contract only; **no visual value, no identifier**) is
...
**CDS-WP-021 — Adaptive Layout and Responsive Foundation is `Completed` / `Closed`**
...
`01145b8a0ad2a68c4c2743205f96ec34f3c2ed15`**.
**`CDS-WP-022` — Theme and Environmental Presentation Model is the currently
