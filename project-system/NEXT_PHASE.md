...
- **Current work package: `CDS-WP-022` — Theme and Environmental Presentation
  ...
  **no** value selection, **no** Source Set, **no** Product Profile, **no** maturity
  ...
  as execution history**. The decisions are recorded as the **prepared** **`DEC-S-137`**
  (with **`ADR-0007`**, covering `DEC-S-137` **only**) and **`DEC-S-138`**, all
  **`PREPARED / HUMAN-MAINTAINER APPROVED / NOT EFFECTIVE` until, and effective from,
  the Human-Maintainer exact-object integration commit of the reviewed CDS-WP-022
  object** — **136 decisions and 6 ADRs until it, 138 and 7 from it**, with the risk
  register at **98** throughout. **Supported Theme Resolution Contexts: 0 before that
  commit, 2 — `Light` and `Dark`, with no default — from it.** **CDS-WP-022 is not
...
- **Previous work package:** **CDS-WP-020 — Reference and Semantic Token
  ...
  **CDS-WP-020 activated no successor**; **CDS-WP-021**
  ...
  **ADR-0005**; it is **not** a numbered work package, is **effective at the
  Human-Maintainer integration commit `2cb244e889c1a6b5a278afb233995a0379b5d9ef`**,
...
- **CDS-WP-020 state:** **Closed** — executed with result `DECISION_REQUIRED`;
  ...
  it records the **seven** normative choices (**OD-1 … OD-7**) that gate every
  ...
  **`DECISION_REQUIRED` stands as the executed result**: closure answers **no** open
  ...
  authority** — see the Step-9 entry below, **effective at `2cb244e8…`** —
...
- **`FR-N-03` state:** **RESOLVED BY EXPLICIT AUTHORING-WP DESTINATION.** Nova
  ...
  and Value Authoring work is routed to **`CDS-WP-020A` — Visual Token Source
  Authoring and Source Set Realization**, and **CDS-WP-024 keeps its validation and
  conformance boundary unchanged**. **Routing is not repair, and registration is not
  activation.**
- **`CDS-WP-020A` state:** **`Planned`, not active, not authorized, work not
  started.** An **inserted** identifier on the `CDS-WP-001A` precedent; **no work
  package was renumbered** and the numeric sequence CDS-WP-017 … CDS-WP-053 is
  unchanged. See [Work Packages](WORK_PACKAGES.md) for its scope and exclusions.
- **CDS-WP-020 Decision Integration Pass (2026-08-27):** separately authorized;
  ...
  **no** work package; and **did not itself close CDS-WP-020**. *(Its effectivity
  qualification was not reconciled by the closure and routing pass; the separately
  authorized post-integration effectiveness reconciliation has since done so — see
  `F-020C-01` in the
  [Post-Candidate Development Roadmap](../docs/roadmap/POST_CANDIDATE_DEVELOPMENT_ROADMAP.md).)*
- **CDS-WP-021 state:** **`Completed` / `Closed`** — **executed with
  ...
  **closed by a separate, separately authorized Human-Maintainer act** whose
  **closure is effective at the Human-Maintainer exact-object integration
  commit `01145b8a0ad2a68c4c2743205f96ec34f3c2ed15`**. **The lifecycle status and
  the execution result are separate axes**: the result stays **`COMPLETE WITH
  NOTES`** and is not rewritten to `COMPLETE`, and the initial `DECISION_REQUIRED`
  execution history stands. Adaptive Layout and Responsive
  ...
  **`WP021-D2` is DEFERRED** — **VF-4 technical root OPEN, VF-4 Source Set identity
  OPEN**, and **no Decision and no ADR exists for it** — **the prepared `DEC-S-137`
  and `ADR-0007` record `WP022-D1`, not `WP021-D2`, and are unrelated to it** — and
  **no `RISK-099` exists.** **Closure does not resolve it**: the authorized
...
- **CDS-WP-022:** **`AUTHORIZED` / `ACTIVE FOR EXECUTION`** — Theme and
  ...
  `DECISION_REQUIRED` **stands as execution history**. The decisions are recorded as the
  **prepared** **`DEC-S-137` — Theme Resolution and Context-Evidence Architecture**
  ...
  **Source Set remaining the sole independently evaluable maturity unit**; and
  context-specific evidence bound to (`sourceSetId`, `sourceRevision`) while recording
  the **Resolver / Composition revision** and the **Theme Resolution Context** as exact
  evidence inputs), with **`ADR-0007`** covering **`DEC-S-137` only**; and **`DEC-S-138`
  — Core Theme Context, Environmental Selection and Fail-Closed Policy** (**`Light` and
  `Dark`** as equal peers with **no default**; **forced colours as an environmental
  accessibility condition and not a Core context**; **explicit viewer choice over
  inferred environment preference**, with mandatory platform accessibility conditions
  **outside** Theme precedence and always binding; and **no default or fallback Theme,
  failing closed** on missing, unsupported and unresolved-conflict resolution, with
  **`Not Applicable`** where Theme resolution genuinely does not apply), which has **no
  ADR**. **All three are `PREPARED / HUMAN-MAINTAINER APPROVED / NOT EFFECTIVE` until,
  ...
  DECISION`**. **136 decisions and 6 ADRs until that commit; 138 and 7 from it; the risk
...
The CDS-WP-019 sequence and the phase transition are **complete**: the architecture
was integrated by the Human-Maintainer commit
`538fbccbf6f554de3b872e9fb75a70d13318feb6`, and DEC-S-127 became effective with the
Human-Maintainer commit `e956560be2b5650cc1ce717931f3da39a2414a1b`. **CDS-WP-020 was
then authorized separately** and has been executed. The remaining sequence is:

| # | Step | Authority | State |
| --- | --- | --- | --- |
| 1 | **Fresh independent review** of the CDS-WP-020 working-tree object | Reviewer ≠ executor | **Done** |
...
| 9a | **CDS Step-9 Decision Integration Pass** — prepare `DEC-S-132 … DEC-S-135` and `ADR-0005` | Claude, under Human-Maintainer authorization | **Done** — object prepared |
| 9b | **Fresh independent review** of the Step-9 Decision Integration object | Reviewer ≠ executor | **Done** |
...
| 18 | **Closure of CDS-WP-021** — a **separate** Human-Maintainer act, not implied by step 16 or step 17 | Human Maintainer | **Done — separately authorized** |
| 18a | **Bounded closure preparation** — record the completed / closed CDS-WP-021 lifecycle state in the live current-authority carriers | Claude, under that authorization | **Done** — closure object **prepared and uncommitted**; **preparation is not closure** |
| 18b | **Fresh independent review** of the closure object, in a new session | Reviewer ≠ executor | **Done** |
| 18c | **Nova final adjudication** of that review | Nova — recommendation only, never approval | **Done** |
| 18d | **Human-Maintainer exact-object integration commit** of the closure object | Human Maintainer | **Done** — `01145b8a0ad2a68c4c2743205f96ec34f3c2ed15` (2026-09-12) |
| 18e | **CDS-WP-021 closure becomes effective** | Effective at step 18d | **Done** — effective at `01145b8a…`; execution result unchanged at **`COMPLETE WITH NOTES`** |
| 19 | **Authorization of the next work package** — a separate, explicit Human-Maintainer decision, **not** implied by step 18e | Human Maintainer | **Done** — **`CDS-WP-022` — Theme and Environmental Presentation Model authorized**; **`CDS-WP-020A` and CDS-WP-023 … CDS-WP-053 remain not authorized** |
| 20 | **Execution of CDS-WP-022** | Claude, under that authorization | **Done** — result **`DECISION_REQUIRED`**, escalating **`WP022-D1` … `WP022-D5`**; object **prepared and uncommitted** |
| 21 | **Human-Maintainer decision on `WP022-D1` … `WP022-D5`** | Human Maintainer | **Done** — 2026-09-12. **All five APPROVED**: Resolver-Modifier Context; `Light` + `Dark` with no default; forced colours as a platform condition; explicit viewer choice > inferred environment preference; no default and fail closed |
| 21a | **Bounded decision rework** — apply the five decisions and prepare `DEC-S-137`, `DEC-S-138` and `ADR-0007` | Claude, under that authorization | **Done** — result **`COMPLETE WITH NOTES`**; object **prepared and uncommitted**; **preparation is not effectivity** |
| 22 | **Fresh independent review** of the reworked CDS-WP-022 object, in a new session | Reviewer ≠ executor | **Required — pending** |
| 23 | **Nova adjudication** of that review | Nova — recommendation only, never approval | **Required — pending** |
| 24 | **Human-Maintainer exact-object integration commit** of the CDS-WP-022 object | Human Maintainer | **Required — pending** |
| 25 | **`DEC-S-137`, `DEC-S-138` and `ADR-0007` become effective** | Effective at step 24 | **Effective only at step 24** — the registers move from 136/6 to 138/7 there, and the risk register stays at 98 |
...
**step 18 does not reach step 18e — authorizing and preparing a closure is not
closing**, **step 18e did not reach step 19 — the CDS-WP-022 authorization was an
independent Human-Maintainer act**, **step 20 does not reach step 21**, **step 21
does not reach step 21a — deciding is not applying**, **step 21a does not reach step
22 — preparing is not reviewing**, **step 24 does not reach step 26 — integration is
not closure**, **step 25 is reached only at step 24 and by nothing earlier**,
and **no
...
mechanism**, so the `DEC-S-135` gate is **not** cleared, **TS-1 still binds**, and
**VP-3, VP-5, VP-6 and VP-7 stay unsatisfied.**

### Why step 5 exists, and what it has produced so far

CDS-WP-020 returned **`DECISION_REQUIRED`**. Its contract half is delivered; its
value and machine-readable half was gated on seven normative choices that no
committed CDS source had made. **Claude created no Decision and no ADR on its own
authority** — both are Human-Maintainer instruments, and the four Decisions and one
ADR now in the working tree exist **only** because the Human Maintainer separately
authorized the Decision Integration Pass on **2026-08-27**.

That pass prepared **DEC-S-128** (one canonical `srgb` colour representation; OKLCH
derivational only), **DEC-S-129** (**WCAG 2.2** contrast evaluation authority;
...
`dimension`, `number`; **`profileVersion` stays `1`**), **DEC-S-131** (the **Source
Set** as the independently evaluable unit; **AGGREGATED is not MATURE**) and
**ADR-0004**. **All five are effective** — effectivity occurred only at the
Human-Maintainer exact-byte integration commit
...
Decision Integration Pass below, which is **effective at `2cb244e8…`**. **What
...
package.** **VP-4 is satisfied for VF-1, VF-2, VF-3, VF-5 and VF-6;
VP-3, VP-5, VP-6 and VP-7 stay unsatisfied**, and VP-2 stays unsatisfied
...
**The relabel was a prerequisite, not an authorization.** It granted no maturity,
admitted no evidence, created no visual value, registered no capability, and
**activated no work package.** CDS-WP-020 was authorized afterwards by a **separate
Human-Maintainer decision**, exactly as this note required.
...
The **Nova Candidate Maturity Review** was then opened on the strength of that GO
and returned **NO-GO**: the normative **Candidate Accessibility Gate was unmet**. A
read-only gap assessment **confirmed** it — **9/9 requirements not demonstrated as
satisfied** — and the Human Maintainer **authorized the CDS-WP-016 Candidate
...
## Previous work package — CDS-WP-019 (closed)
...
CDS-WP-019 defined **how** the visual foundation is structured. CDS-WP-020 defines
**what a reference primitive and a semantic role are, and what each must declare** —
the contract for token-flow layers 1 and 2. It was authorized by a separate
Human-Maintainer decision on 2026-08-26 and has been executed.

### Objective of CDS-WP-020

- Establish the
  [Visual Reference Token Foundation](../docs/architecture/VISUAL_REFERENCE_TOKEN_FOUNDATION.md) —
  token-flow layer **1**: what a reference primitive is and is not, which families
  hold one, ten reference obligations, seven requirements on any future scale, the
  reference naming rules, provenance and lifecycle, the theme and profile boundary,
  and ten validation requirements handed to CDS-WP-024.
- Establish the
  [Visual Semantic Token Foundation](../docs/architecture/VISUAL_SEMANTIC_TOKEN_FOUNDATION.md) —
  token-flow layer **2**: role classes per family, twelve universal role
  obligations, the **alias model**, the semantic naming rules, channel and product
  neutrality, seven theme-compatibility requirements, the Semantic Status boundary,
  the **focus role set**, state-role constraints, and fifteen validation
  requirements.
- Establish the
  [Visual Token Value Selection Rules](../docs/governance/VISUAL_TOKEN_VALUE_SELECTION_RULES.md) —
  the prerequisites, the twelve-criterion evaluation, the inadmissible grounds, the
  value record, the accessibility constraints, the validation strategy, and the
  lifecycle disposition.
- Record the **non-normative**
  [Visual Token Foundation Open Decisions](../docs/roadmap/VISUAL_TOKEN_FOUNDATION_OPEN_DECISIONS.md)
  register — **OD-1 … OD-7**, each with proposition, alternatives, recommendation,
  affected files, and track.
- **Register no new Decision.** Every binding statement is an application of a
  decision already in force.
- Record **CDS-WP-019 as closed** and **CDS-WP-020 as the current authorized work
  package**, leaving **CDS-WP-021 … CDS-WP-053** `Planned`, not active, not
  authorized.

### The result, stated plainly

**`DECISION_REQUIRED`.** The **contract** half is delivered. The **value** and
**machine-readable** half is gated on seven normative choices no committed CDS
source has made: the colour space (**OD-1**), the admitted `$type` set (**OD-2**),
the visual source-set identity and topology (**OD-3**), the concrete identifier
grammar (**OD-4**), scale topology (**OD-5**), the role vocabulary and family
granularity (**OD-6**), and the sequencing against CDS-WP-022 (**OD-7**).

**The machinery is sufficient; the decisions are not.** Nothing in the format
profile, the source-set classes, the committed schemas, the offline validator, or
the serialization contract blocks a visual source set. Writing one anyway would have
settled OD-1 … OD-5 by implication — **acquiring** authority rather than receiving
it, which DEC-S-033 prohibits.

### CDS-WP-020 explicitly establishes none of the following

- **no colour, colour space, palette, typeface, font stack, weight, size, line
  height, tracking, spacing step, radius, stroke, shadow, opacity level, elevation
  step, icon dimension, motion value, breakpoint, or theme instance**;
- **no identifier** created, adopted, reserved, or recommended for adoption;
- no token source file, manifest, resolver, schema, validator rule, diagnostic,
  test, fixture, component, channel adapter, or asset;
- no named extension point, no Product Profile, no brand, and no product identity;
- no evidence produced or admitted; **no maturity change**; no Candidate or Stable
  promotion; no claim or conformance; no CoreOps pilot start; no consumer
  integration; no licence; no publication, release, or tag;
- **no risk entry** — the Risk Register stays at **98** with **no risk accepted,
  closed, or re-scored**, and **`RISK-099` is not required**. *(At the original
  CDS-WP-020 milestone this bullet also read "no Decision, no ADR"; the separately
  authorized 2026-08-27 Decision Integration Pass registered **DEC-S-128 …
  DEC-S-131** and **ADR-0004**, which are **effective** — at the integration
  commit `42a568d8…` the effective registers became **DEC-S-131** and **ADR-0004**,
  and the CDS Step-9 Decision Integration Pass has since advanced them to
  **DEC-S-135** and **ADR-0005** at commit `2cb244e8…`.)*;
- **no phase rename**; **no capability registration** for audio/sonic, haptic,
  multimodal, AI/agent, or safety subject matter;
- **no activation of CDS-WP-021 or any later work package**, and **no change to
  CR-004's registered Layer-5 mapping**;
- no change to `tokens/**`, `schemas/**`, `tools/**`, `tests/**`, `artifacts/**`,
  `docs/foundations/**`, or `docs/risks/**`, and **no change to
  any Semantic Status source, revision, maturity, approval, or evidence package**.
  **`AE1-CDS-WP016-SEMSTATUS-004` was not transferred to anything.**
  *(`docs/decisions/**` was also untouched at the original CDS-WP-020 milestone. The
  separately authorized 2026-08-27 Decision Integration Pass added `ADR-0004` and
  the DEC-S-128 … DEC-S-131 entries there; every other frozen domain above remains
  untouched.)*

### Still prohibited in the current phase

*(Unchanged by CDS-WP-019, CDS-WP-020, CDS-WP-021 and CDS-WP-022. An
**architecture** for the visual foundation is not concrete visual design; a
**contract** for what a colour role must declare is not selecting a colour; a
**contract** for what a presentation context is is not supporting one; and recording
an open decision is not making it. **Unchanged by DEC-S-127 as well:** a phase
transition grants no authority, so every prohibition below survives the relabel
intact and is lifted only by a separately authorized work package. **Selecting theme
values stays prohibited**, in both temporal states: **supported Theme Resolution
Contexts are 0 before the CDS-WP-022 exact-object integration commit and 2 — `Light`
and `Dark` — from it, with neither the default.** **A supported context is not a theme
value**, **`Light` and `Dark` are human-readable architectural names and not
machine-readable identifiers**, and **supporting them authorizes no theme-value
selection**: **VP-3, VP-5, VP-6 and VP-7 stay `UNSATISFIED`**, and **visual values
stay 0.**)*
...
- creating a new work-package ID beyond the recorded CDS-WP-017 … CDS-WP-053
  planning sequence without Human-Maintainer approval. *(**`CDS-WP-020A`** was
  registered **with** that approval, as the Human-Maintainer-approved Nova
  adjudication of **`FR-N-03`**. It is an **inserted** identifier that renumbered
  nothing, and **registering it activated nothing**.)*
...
Human-Maintainer act at step 10 — **executed with result `COMPLETE WITH NOTES`**,
**integrated** at `a6bd7bf0c290886bbe2695c0f9cf70efbef3f1e9`, and **then closed by a
further separate Human-Maintainer authorization at step 18**, whose closure is
...
all **not effective until the Human-Maintainer exact-object integration commit of the
reviewed object** — **136/6 until it, 138/7 from it, risks 98 throughout**; and **its
closure is a separate Human-Maintainer act that has not occurred.**
**`DEC-S-136` and `ADR-0006` were prepared under the execution
authorization and became effective at that integration commit** — the
effective registers are now **136 decisions and 6 ADRs**, and **closure changes
neither their effectivity nor their effectivity commit**. **`PREPARED ≠
EFFECTIVE`** held until the commit, and **`EFFECTIVE ≠ CLOSED`** held until the
closure act: integration closed no work package.
**Planning is not authorization:** recording `CDS-WP-020A` and CDS-WP-023 …
...
other**, and **authorizing CDS-WP-022 satisfied no `CDS-WP-020A` prerequisite.** **Execution is not closure:** uncommitted executor output changes no
...
- [Visual Foundation Theme Architecture](../docs/architecture/VISUAL_FOUNDATION_THEME_ARCHITECTURE.md) — CDS-WP-019 and CDS-WP-022 entry point for VF-9
- [Visual Token Value Selection Rules](../docs/governance/VISUAL_TOKEN_VALUE_SELECTION_RULES.md)
- [Visual Token Foundation Open Decisions](../docs/roadmap/VISUAL_TOKEN_FOUNDATION_OPEN_DECISIONS.md) — **non-normative**; OD-1 … OD-7
