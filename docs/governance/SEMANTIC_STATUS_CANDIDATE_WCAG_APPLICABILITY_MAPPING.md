...
— independently reviewed **PASS** as clean-HEAD Evidence 002 and admitted at AE-1 as
`AE1-CDS-WP016-SEMSTATUS-002` for source revision `semantic-status-rev-0001`.
**The currently admitted package is `AE1-CDS-WP016-SEMSTATUS-004`**, bound to the
current source revision `semantic-status-rev-0002-candidate`, source scope only
([record](SEMANTIC_STATUS_AE1_004_ADMISSION_RECORD.md);
[evidence record](../operations/SEMANTIC_STATUS_CANDIDATE_AE1_004_EVIDENCE_RECORD.md)).
**Admitted structural evidence is not a criterion pass**; see the claim boundary
below. **Current WCAG conformance: NONE.** All applicability classifications in this
...
| Criteria for which any evidence exists today | **5** — Group A only, and only its source-level part, only at **admitted AE-1** (`AE1-CDS-WP016-SEMSTATUS-004`, source scope) |
